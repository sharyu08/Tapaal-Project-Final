...
export function EditOutwardMail({ mailId, onBack, onSave, onRefresh }: EditOutwardMailProps) {
  ...
  const fetchMailDetail = async () => {
    try {
      ...
      if (response.success) {
        ...
        setSentBy(mailData.sentBy || '');
        setReceiver(mailData.receiver || '');
        setReceiverAddress(mailData.receiverAddress || '');
        setSubject(mailData.subject || '');
        setDetails(mailData.details || '');
        setDepartment(mailData.department || '');
        setPriority(mailData.priority || 'Normal');
        setSentDate(mailData.date || '');
        setDueDate(mailData.dueDate || '');
        ...
        setCost(mailData.cost?.toString() || '');
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      console.log('📤 Updating outward mail:', mailData);

      const response = await outwardMailService.updateOutwardMail(mailId, mailData);
      ...
      if (response.success) {
        ...
        setError('Failed to update outward mail: ' + response.message);
      }
    } catch (error) {
      console.error('💥 Error updating outward mail:', error);
      setError('Failed to update outward mail. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">Edit Outward Mail</h1>
